...
def predict_points(data):
    ...
    model = Lasso(alpha=0.1)  # Adjust alpha for regularization strength (higher = stronger regularization)
    model.fit(X_train_scaled, y_train)
    
    # Predicting on the test set
    predictions = model.predict(X_test_scaled)
    mse = mean_squared_error(y_test, predictions)  # Optionally calculate MSE
    st.write(mse)
    # Predicting 2024 points for all drivers
    ...
    st.write("""
    <div style="background-color: black; padding: 10px;">
        <span style="color: white;">Note: Predicting sports results are not always accurate. I have used a Ridge regression model to predict the data with the use of the last 3 years of points data, driver skill rating, car rating, and constructor strategy.</span>
    </div>
    """, unsafe_allow_html=True)
#---------------------------------------------------------------------------------------
    st.write('---------------------------------------------------------------------------------------')
